...
const CATS = [
  ...
  { id:'tts', label:'Speech', icon:'record_voice_over', badge:'cb-tts' },
  { id:'music', label:'Music', icon:'music_note', badge:'cb-music' },
...
function buildMsgEl(msg) {
  ...
  } else if (!isUser && !msg.imageUrl && !msg.videoUrl && !msg.audioUrl) {
...
async function sendMessage() {
  ...
  try {
    ...
    } else if (S.selectedCat === 'tts') {
      await callTtsAPI(chatId, text);
    } else if (S.selectedCat === 'music') {
      await callMusicAPI(chatId, text);
...
async function callTtsAPI(chatId, text) {
  const voice = S.ttsVoice || (S.selectedModel.voices && S.selectedModel.voices[0]) || 'Rachel';
  const payload = await fetchAudioGeneration({
    model: S.selectedModel.id || 'eleven-turbo',
    voice,
    input: text || 'Hello from OrchidLLM',
  }, S.apiMode, S.byopKey);

  const url = payload instanceof Blob ? URL.createObjectURL(payload) : (payload?.url || null);
  if (!url) throw new Error('No audio returned');
  ...
  const aiMsg = {
    role: 'assistant',
    audioUrl: url,
    content: `🗣️ Speech synthesized with ${S.selectedModel.name} · voice: ${voice}.`,
    time: ts(),
    model: S.selectedModel.name,
  };
  addMessage(chatId, aiMsg);
...
async function callMusicAPI(chatId, text) {
  const payload = await fetchAudioGeneration({
    model: S.selectedModel.id || 'suno-v4',
    input: text || 'An upbeat instrumental track',
  }, S.apiMode, S.byopKey);

  const url = payload instanceof Blob ? URL.createObjectURL(payload) : (payload?.url || null);
  if (!url) throw new Error('No audio returned');
  // A generated "cover" for the track (reuses the stub image generator).
  const cover = getImageUrl(text || 'album cover art, abstract, vibrant', S.selectedModel.id, {}, S.apiMode, S.byopKey);
  ...
  const aiMsg = {
    ...
    imageUrl: cover,
    audioUrl: url,
    content: `🎵 Generated a track with ${S.selectedModel.name} — “${(text || 'Untitled').slice(0, 60)}”.`,
    time: ts(),
    model: S.selectedModel.name,
  };
  addMessage(chatId, aiMsg);
...
function applyTierColors() {
  ...
  const accent = TIER_COLORS[tier] || TIER_COLORS.free;
  const gradientEnd = TIER_GRADIENT_ENDS[tier] || TIER_GRADIENT_ENDS.free;
  
  const root = document.documentElement;
  root.style.setProperty('--tier-accent', accent);
  root.setAttribute('data-active-tier', tier);
  
  // Override primary color variables to reflect the active tier
  root.style.setProperty('--p', accent);
  root.style.setProperty('--t', gradientEnd);
  root.style.setProperty('--on-p', onPrimaryColors[tier] || '#ffffff');
  
  // Map standard MD3 variables
  root.style.setProperty('--md-sys-color-primary', accent);
  root.style.setProperty('--md-sys-color-on-primary', onPrimaryColors[tier] || '#ffffff');
  
  const isDark = S.theme === 'dark';
  const containerColor = isDark 
    ? `color-mix(in srgb, ${accent} 16%, #14101F)`
    : `color-mix(in srgb, ${accent} 12%, #EDE9FE)`;
  const onContainerColor = isDark
    ? accent
    : `color-mix(in srgb, ${accent} 85%, #000)`;
    
  root.style.setProperty('--pc', containerColor);
  root.style.setProperty('--on-pc', onContainerColor);
  root.style.setProperty('--md-sys-color-primary-container', containerColor);
  root.style.setProperty('--md-sys-color-on-primary-container', onContainerColor);
}
...
function setAuthUi(session) {
  const localSession = JSON.parse(localStorage.getItem('orchid_session') || 'null');
  ...
  if (trigger) {
    trigger.onclick = activeSession && activeSession.authenticated
      ? () => toggleAuthDropup()
      : () => { window.location.href = 'https://vendouple.github.io/OrchidLLM_Frontend_Demo/login'; };
  }
  ...
  if (activeSession && activeSession.authenticated) {
    const u = activeSession.user || {
      ...
      display_name: activeSession.display_name || activeSession.username || 'johndoe'
    };
    const tier = activeSession.tier || 'free';
    const tierLabel = tier.charAt(0).toUpperCase() + tier.slice(1) + ' Tier';
    const dashboardHref = '/users.html';
    const dashboardLabel = 'Go to Dashboard';

    // Trigger row
    if (authName) authName.textContent = u.display_name || u.username;
    if (authSub) authSub.textContent = tierLabel;
    
    // Display initial in avatar wrap if no avatar image
    const avatarWrap = document.getElementById('auth-avatar-wrap');
    if (avatarWrap) {
      avatarWrap.style.border = '2px solid transparent';
      avatarWrap.style.transition = 'all var(--dur-short) ease';
      avatarWrap.onmouseover = () => { avatarWrap.style.borderColor = 'var(--tier-accent)'; };
      avatarWrap.onmouseout = () => { avatarWrap.style.borderColor = 'transparent'; };
    }
    ...
    if (authIcon) {
      if (u.avatar) {
        ...
        authIcon.textContent = (u.display_name || u.username || 'J')[0].toUpperCase();
        authIcon.style.fontSize = '14px';
        authIcon.style.fontWeight = '800';
      }
    }
    if (authImg) {
      if (u.avatar) {
    ...
    if (loggedIn) {
      ...
      if (du) du.textContent = u.display_name || u.username;
      
      const dropupAvatarIcon = document.getElementById('dropup-avatar-icon');
      if (da) {
        if (u.avatar) {
          da.src = u.avatar;
          da.style.display = 'block';
          if (dropupAvatarIcon) dropupAvatarIcon.style.display = 'none';
        } else {
          da.style.display = 'none';
          if (!dropupAvatarIcon) {
            const wrap = da.parentElement;
            const fallback = document.createElement('div');
            fallback.id = 'dropup-avatar-icon';
            fallback.textContent = (u.display_name || u.username || 'J')[0].toUpperCase();
            Object.assign(fallback.style, {
              width: '36px', height: '36px', borderRadius: '50%',
              background: 'linear-gradient(145deg, var(--p), var(--t))',
              color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontWeight: '800', fontSize: '16px', boxShadow: 'var(--sh1)'
            });
            wrap.insertBefore(fallback, da);
          } else {
            dropupAvatarIcon.style.display = 'flex';
            dropupAvatarIcon.textContent = (u.display_name || u.username || 'J')[0].toUpperCase();
          }
        }
      }
      if (dr) dr.textContent = tierLabel;
      if (dal) dal.style.display = activeSession.isAdmin ? 'flex' : 'none';
      if (dul) {
        dul.style.display = 'flex';
        dul.href = '/users.html';
      }
    }
    ...
    if (authSub) authSub.textContent = 'Open the login page';
    if (authIcon) {
