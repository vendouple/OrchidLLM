/**
 * OrchidLLM Login page.
 * Handles view transitions and the two real entry points: GitHub OAuth
 * (full redirect handled server-side) and Try Demo (server-issued demo key, §17a).
 */
...
(function () {
  ...
  // --- GitHub OAuth (real) ---
  // Real OAuth is a full page navigation away to GitHub and back, so there is no
  // client-side "authenticating" delay to fill — just show the loading view for the
  // instant before the browser leaves, then hand off. The server-side callback
  // upserts the user and redirects straight to the dashboard on success.
  btnGitHub.addEventListener('click', () => {
    switchView('loading');
    loadingText.textContent = 'Redirecting to GitHub…';
    window.location.href = '/Account/ExternalLogin?provider=GitHub';
  });
  ...
  btnStartDemo.addEventListener('click', () => {
    ...
    const platform = /Mobi|Android/i.test(navigator.userAgent) ? 'web_mobile' : 'web_desktop';

    fetch('/Account/TryDemo', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ platform }),
    })
      .then((r) => {
        if (!r.ok) throw new Error('demo session request failed');
        return r.json();
      })
      .then((data) => {
        showSnackbar(`Demo key: ${data.demoKeyPreview}…`);
        switchView('success');
        successUsername.textContent = 'Demo User';

        // Demo users go to chat (home page)
        setTimeout(() => {
      ...
      .catch(() => {
        switchView('demo');
        showSnackbar('Could not start a demo session. Please try again.');
      });
  });
