...
async function loadCanonicalCatalogSnapshot() {
    const modelResult = await executeQuery(`
        ...
        WHERE cm.is_active = 1
          ...
          AND (NVL(cm.deprecates_at, cm.deprecation_date) IS NULL OR NVL(cm.deprecates_at, cm.deprecation_date) > CURRENT_TIMESTAMP)
        ORDER BY cm.category ASC, NVL(cm.display_name, cm.name) ASC, NVL(cm.model_slug, cm.model_id) ASC
    `);
    ...
    const [tierResult, paramResult, whitelistResult, routeResult, routeParamResult, aliasResult] = await Promise.all([
        ...
        executeQuery(`
            SELECT
                ...
                provider_model_id,
                route_label,
                NVL(provider_context_window, context_window) AS context_window,
                timeout_ms,
                priority,
                supports_batch,
                supports_caching,
            ...
            ORDER BY canonical_model_id ASC, priority DESC, NVL(provider_context_window, context_window) ASC NULLS LAST, id ASC
        `, bindVars),
        executeQuery(`
            ...
            ORDER BY canonical_route_id ASC, param_name ASC
        `, bindVars),
        executeQuery(`
            SELECT canonical_model_id, alias_model_id, is_primary_alias, notes
            FROM canonical_model_aliases
            WHERE canonical_model_id IN (${placeholders})
              AND is_active = 1
            ORDER BY is_primary_alias DESC, alias_model_id ASC
        `, bindVars)
    ]);
    ...
    for (const row of aliasResult.rows || []) {
        ...
        aliasesById.set(alias.aliasModelId, Number(modelId));
    }
...
async function loadCanonicalMappingSnapshot() {
    const result = await executeQuery(`
        SELECT
            ...
            NVL(route.provider_context_window, route.context_window) AS context_window,
            route.timeout_ms,
            route.priority,
        ...
        WHERE route.is_active = 1
          ...
          AND (cm.expires_at IS NULL OR cm.expires_at > CURRENT_TIMESTAMP)
          AND (NVL(cm.deprecates_at, cm.deprecation_date) IS NULL OR NVL(cm.deprecates_at, cm.deprecation_date) > CURRENT_TIMESTAMP)
        ORDER BY NVL(cm.model_slug, cm.model_id) ASC,
                 LOWER(route.provider_name) ASC,
                 route.priority DESC,
                 NVL(route.provider_context_window, route.context_window) ASC NULLS LAST,
                 route.id ASC
    `);
