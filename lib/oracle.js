...
export async function executeQuery(sql, binds = {}, options = {}) {
    ...
    // Normalise out-bind specs: callers may pass { dir: 'out', type: 'NUMBER' }
    // using string constants.  Convert them to the real oracledb enum values so
    // the driver doesn't reject or silently ignore them.
    const resolvedBinds = {};
    for (const [key, val] of Object.entries(binds)) {
        if (val && typeof val === 'object' && val.dir !== undefined && !Buffer.isBuffer(val)) {
            const spec = { ...val };
            // dir
            if (spec.dir === 'out' || spec.dir === 'BIND_OUT')  spec.dir = oracledb.BIND_OUT;
            if (spec.dir === 'in'  || spec.dir === 'BIND_IN')   spec.dir = oracledb.BIND_IN;
            if (spec.dir === 'inout' || spec.dir === 'BIND_INOUT') spec.dir = oracledb.BIND_INOUT;
            // type
            if (spec.type === 'NUMBER')  spec.type = oracledb.NUMBER;
            if (spec.type === 'STRING')  spec.type = oracledb.STRING;
            if (spec.type === 'DATE')    spec.type = oracledb.DATE;
            if (spec.type === 'CURSOR')  spec.type = oracledb.CURSOR;
            if (spec.type === 'BUFFER')  spec.type = oracledb.BUFFER;
            if (spec.type === 'CLOB')    spec.type = oracledb.CLOB;
            if (spec.type === 'BLOB')    spec.type = oracledb.BLOB;
            resolvedBinds[key] = spec;
        } else {
            resolvedBinds[key] = val;
        }
    }
    ...
    try {
        ...
        const result = await connection.execute(sql, resolvedBinds, {
            autoCommit:  options.autoCommit !== false,
            callTimeout: options.callTimeout ?? CALL_TIMEOUT_MS,
            ...options,
        });

        // OracleDB's OUT_FORMAT_OBJECT rows carry internal NVPair objects that
        // create circular references when passed to JSON.stringify().
        // Strip them by copying only own enumerable properties to plain objects.
        // Also properly read CLOB/BLOB Lob objects via getData() instead of toString().
            ...
            rows = await Promise.all(rows.map(async (r) => {
                ...
                for (const key of Object.keys(r)) {
                    ...
                    if (v === null || v === undefined) {
                        ...
                        plain[key] = v.toISOString();
                    } else if (typeof v === 'object' && typeof v.getData === 'function') {
                        // Oracle Lob (CLOB/BLOB) — read full content as string
                        try {
                            plain[key] = await v.getData();
                        } catch (lobErr) {
                            console.error(`[oracle] Lob.getData() error for column ${key}:`, lobErr.message);
                            plain[key] = null;
                        }
                    } else if (typeof v === 'object' && typeof v.toString === 'function' && v.constructor && v.constructor.name !== 'Object' && v.constructor.name !== 'Array') {
                        // Other Oracle internal types (NVPair etc.) — coerce to string
                        plain[key] = v.toString();
                    } else {
                        plain[key] = v;
                    }
                }
