...
export async function POST(request: NextRequest) {
  try {
    ...
    const { model, prompt, n, size, response_format } = body;
    ...
    // No valid key - treat as demo mode
    if (sessionId) {
      const compositeId = generateCompositeId(sessionId, clientIP);
...
async function forwardToPollinations(body: ImageGenerationRequest, apiKey: string) {
